...
```
### 4. 絞り込み
部署ごとに、部署番号、部署名、現在のマネージャーの従業員番号、マネージャーのファーストネーム、マネージャーのラストネームを取得
```sql
SELECT dm.dept_no, d.dept_name, dm.emp_no, e.first_name, e.last_name
FROM dept_manager dm
INNER JOIN employees e
...
### 5. 給与
従業員番号10001から10010の従業員ごとに、ファーストネーム、ラストネーム、いつからいつまで給与がいくらだったかを取得
```sql
SELECT s.emp_no, e.first_name, e.last_name, MIN(s.from_date), MAX(s.to_date), SUM(s.salary)
FROM salaries s
INNER JOIN employees e
...
GROUP BY s.emp_no;
+--------+------------+-----------+------------------+----------------+---------------+
| emp_no | first_name | last_name | MIN(s.from_date) | MAX(s.to_date) | SUM(s.salary) |
+--------+------------+-----------+------------------+----------------+---------------+
|  10001 | Georgi     | Facello   | 1986-06-26       | 9999-01-01     |       1281612 |
|  10002 | Bezalel    | Simmel    | 1996-08-03       | 9999-01-01     |        413127 |
|  10003 | Parto      | Bamford   | 1995-12-03       | 9999-01-01     |        301212 |
|  10004 | Chirstian  | Koblick   | 1986-12-01       | 9999-01-01     |        904196 |
|  10005 | Kyoichi    | Maliniak  | 1989-09-12       | 9999-01-01     |       1134585 |
|  10006 | Anneke     | Preusig   | 1990-08-05       | 9999-01-01     |        606179 |
|  10007 | Tzvetan    | Zielinski | 1989-02-10       | 9999-01-01     |        991574 |
|  10008 | Saniya     | Kalloufi  | 1998-03-11       | 2000-07-31     |        147923 |
|  10009 | Sumant     | Peac      | 1985-02-18       | 9999-01-01     |       1409122 |
|  10010 | Duangkaew  | Piveteau  | 1996-11-24       | 9999-01-01     |        460338 |
+--------+------------+-----------+------------------+----------------+---------------+
...
### 6. 内部結合と外部結合の違い
- 内部結合
結合条件に指定している値が両方(結合元, 結合先)のテーブルに存在するデータを抽出する結合のこと  
結合先のテーブルに紐付かないデータを出力したくない場合は、内部結合を用いる

- 外部結合
結合条件として指定している値が基準(結合元)となるテーブルに存在すれば抽出する結合のこと  
結合先のテーブルに紐付かないデータを出力したい場合は外部結合の結合条件で条件を用いる  
